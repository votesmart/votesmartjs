(function () {
  ...
  VoteSmart.prototype.Location = function () {
    return {
      location: function (constructor, callback) {
  ...
  VoteSmart.prototype._request = function (constructor, callback) {
    ...
    this._ajaxGet(url, function (err, res) {
      ...
      if (json.error) callback(json, null);
  ...
  VoteSmart.prototype._ajaxGet = function (url, callback) {
    ...
    request.onreadystatechange = function () {
      // Set headers before connection is made
      if (request.readyState == 1) {
        request = self._setHeaders(request, self.headers, self.apiKey);
        return;
      }
      // Unexpected HTTP status codes
      if ([200, 206, 404, 416, 666].indexOf(request.status) == -1) return;
      // readyStates we don't have a use for
      if (request.readyState < 4) return;
      // Callback on complete
      if (request.readyState === 4) {
        if ([404,416,666].indexOf(request.status) > -1) {
  ...
  if (typeof module !== 'undefined' && module.exports) {
    ...
    VoteSmart.prototype._request = function (constructor, callback) {
      var opts = {
        url: this._urlConstructor(constructor),
